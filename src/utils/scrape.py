    ...
    try:
        ...
        while to_visit and page_count < max_pages:
            ...
            try:
                ...
                for link in soup.find_all("a", href=True):
                    ...
                    if not full_url.startswith(base_domain) or not full_url.startswith(
                        ("http://", "https://")
                    ):
                        continue
                    ...
                    if (
                        "election" in full_url.lower()
                        or "statistic" in full_url.lower()
                        or "data" in full_url.lower()
                        or "report" in full_url.lower()
                    ...
                    if (
                        ...
                        or "pdf" in link_text
                        or "report" in link_text
                        or "statistic" in link_text
                    ):
                ...
                for form in soup.find_all("form"):
                    ...
                    if (
                        "download" in form_text
                        or "pdf" in form_text
                        or "report" in form_text
                    ):
        ...
        for link in pdf_links:
            ...
            try:
                ...
                is_pdf = "application/pdf" in content_type or link.lower().endswith(
                    ".pdf"
                )

                min_pdf_size = 1000  # Minimum size for a valid PDF (1KB)
                if (
                    pdf_response.status_code == HTTP_OK
                    and is_pdf
                    and len(pdf_response.content) > min_pdf_size
                ):
                    with filename.open("wb") as file:
                        file.write(pdf_response.content)
                    successful_downloads += 1
                    print(
                        f"Successfully downloaded: {filename} ({len(pdf_response.content)} bytes)"
                    )
                else:
                    if pdf_response.status_code != HTTP_OK:
                        print(
                            f"Failed to download: {link}, Status code: {pdf_response.status_code}"
                        )
                    elif len(pdf_response.content) <= min_pdf_size:
                        print(
                            f"Skipping: {link}, Too small to be valid PDF ({len(pdf_response.content)} bytes)"
                        )
                    else:
