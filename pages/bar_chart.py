...
# Import bar chart visualization
from pages import bar_chart, map_viz
...
def create_app():
    ...
    navbar = dbc.NavbarSimple(
        children=[
            ...
            dbc.NavItem(
                dbc.NavLink("Map Visualization", href="/map", style={"color": "black"})
            ),
            dbc.NavItem(
                dbc.NavLink("Bar Chart", href="/bar-chart", style={"color": "black"})
            ),
        ],
    ...
    def display_page(pathname):
        if pathname == "/" or pathname == "/index":
            ...
            return html.Div([navbar, bar_chart.layout])
        else:
            return html.Div(
    ...
    # Register the bar chart visualization callback
    @app.callback(
        Output("bar-chart-graph", "figure"),
        Input("bar-year-dropdown", "value"),
        Input("bar-office-dropdown", "value"),
        Input("bar-state-dropdown", "value"),
    )
    def update_bar_chart_callback(year, office, state):
        return bar_chart.update_bar_chart(year, office, state)
