...
def evaluate_numeric_with_confusion(df_true, df_pred, numeric_cols):
    ...
    for col in numeric_cols:
        for i in range(len(df_true)):
            m, t = compare_digits_with_confusion(df_true[col].iloc[i],
                                                 df_pred[col].iloc[i],
                                                 master_conf)
            overall_matched += m
            overall_total   += t
...
def evaluate_text_with_char_confusion(df_true, df_pred, text_cols, char_list=string.ascii_lowercase):
...
def analyze_char_confusion(conf_mat, char_list):
    ...
    for i, rsum in enumerate(row_sums):
        if rsum == 0: continue
        sorted_preds = sorted(range(len(char_list)), key=lambda x: conf_mat[i][x], reverse=True)
